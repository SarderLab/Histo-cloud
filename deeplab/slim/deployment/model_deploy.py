...
def _optimize_clone(optimizer, clone, num_clones, regularization_losses,
  ...
  if sum_loss is not None:
    with tf.device(clone.device):
      ...
      # clone_grad, _ = tf.clip_by_global_norm(clone_grad, 5.0)
...
def deploy(config,
  ...
  with tf.device(config.optimizer_device()):
    if optimizer:
      ...
      total_loss, clones_gradients = optimize_clones(clones, optimizer, epsilon=epsilon)
...
def _sum_clones_gradients(clone_grads):
  ...
  clipped_grads, _ = tf.clip_by_global_norm([grad for grad, var in sum_grads], 5.0)
  sum_grads = [(clipped_grads[i], v) for i, (_, v) in enumerate(sum_grads)]
  return sum_grads
