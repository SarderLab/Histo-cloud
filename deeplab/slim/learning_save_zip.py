...
def train_step(sess, train_op, global_step, progress_percent, train_step_kwargs):
  ...
  if 'should_log' in train_step_kwargs:
    if sess.run(train_step_kwargs['should_log']):
      ...
      # if np_global_step % eval_interval == 0:
      #   labels, predictions = sess.run([train_step_kwargs['labels'], train_step_kwargs['predictions']])
      #   sess.run([accuracy_update], feed_dict={y_true: labels, y_pred: predictions})
      #   acc = sess.run([accuracy_metric], feed_dict={y_true: labels, y_pred: predictions})

      # logging.info('Accuracy: %.2f%'.format(acc)/)
      
      with ProgressHelper('Train Network') as helper:
        helper.message(f'Loss: {total_loss}')
        # helper.message(f'Accuracy: {acc}:.2f%')
...
def zip_model_if_new(logdir,output_zip):
    ...
    if latest_model is not None:
        ...
        if os.path.isfile(output_zip):
            zip_mod_time = os.path.getmtime(output_zip)
        else: zip_mod_time = 0

        # if newer model exists
        # logging.info('latest model:  [{}]'.format(latest_model))
        # logging.info('zip mod time:  [{}]'.format(zip_mod_time))
        # logging.info('ckpt mod time: [{}]'.format(ckpt_mod_time))
        if ckpt_mod_time > zip_mod_time:
            cwd = os.getcwd()
            os.chdir(logdir)
