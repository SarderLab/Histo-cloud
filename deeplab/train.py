...
with warnings.catch_warnings():
    ...
    from datasets import wsi_data_generator
...
flags.DEFINE_integer('train_crop_size', 1024,
                  'Image crop size [size, size] during training.')

flags.DEFINE_multi_integer('wsi_downsample', 4,
...
flags.DEFINE_integer('num_of_classes', 1, "Number of Classes.")
...
def _build_deeplab(iterator, outputs_to_num_classes, ignore_label):
  ...
  model_options = common.ModelOptions(
      ...
      crop_size=[FLAGS.train_crop_size,FLAGS.train_crop_size],
  ...
  with tf.Graph().as_default() as graph:
    ...
    with tf.device(config.variables_device()):
      total_loss, grads_and_vars = model_deploy.optimize_clones(
          clones, optimizer, epsilon=FLAGS.epsilon)
